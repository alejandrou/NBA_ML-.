...
Use `TestClient` as a context manager when lifespan is present. Keep fixtures local and deterministic, clear `app.dependency_overrides`, and split quick offline HTTP tests from the real-DB integration lane under `tests/integration/`. Never require PostgreSQL, external network, production data, or scraping for basic HTTP tests.
