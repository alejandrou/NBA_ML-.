...
@pytest.mark.unit
def test_parse_player_page_postseason_reads_supported_visible_and_commented_tables() -> None:
    parsed = parse_player_page_postseason(POSTSEASON_FIXTURE.read_text(encoding="utf-8"))

    assert set(parsed) == set(SUPPORTED_PLAYER_PAGE_POSTSEASON_TABLES)
    assert [row["team_id"] for row in parsed["totals"]] == ["BRK"]
    assert parsed["per_game"][0]["pts_per_g"] == "22.8"
    assert parsed["advanced"][0]["per"] == "22.7"
