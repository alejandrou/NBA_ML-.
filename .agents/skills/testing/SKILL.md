...
Use `TestClient` and dependency overrides with deterministic local fixtures. Test the public contract rather than internals: declared and absent fields, bounds, pagination, and error responses. Clear `app.dependency_overrides`, use `TestClient` as a context manager when lifespan applies, and keep HTTP unit tests separate from the PostgreSQL integration lane under `tests/integration/`.
