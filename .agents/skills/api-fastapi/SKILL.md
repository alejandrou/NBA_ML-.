...
- Health is liveness only: `GET /api/v1/health` returns process health without DB Sessions, repositories, network calls, or DB checks. Readiness is a separate route: `docs/architecture/API_CONTRACT.md` fixes its public shape and `docs/architecture/API_ARCHITECTURE.md` its runtime boundaries.
