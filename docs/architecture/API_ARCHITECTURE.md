...
**Connection establishment is bounded separately.** `api_readiness_timeout_seconds` bounds statements, which only start once a connection exists; a host that swallows packets is never reached by it and would hang the request before the probe runs. Engine construction therefore carries its own bound: `database_connect_timeout_seconds`, default `5.0`, constrained to `> 0`, applied as libpq's `connect_timeout` in `create_db_engine()` and so shared by the CLI and loaders. libpq takes whole seconds and treats anything below 2 as 2, so the configured value is rounded up and floored at 2. It is applied only to PostgreSQL URLs, since it is a libpq keyword no other driver accepts. Worst-case readiness latency against a dead host is therefore the two bounds in sequence, and a connect that times out is a connection failure — `"Database unavailable"`, not the timed-out body, which stays reserved for a database that answered the handshake and then stalled.
